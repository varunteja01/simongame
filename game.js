...
//2.5 creating a new empty array called gamePattern
var gamePattern=[];
//2.3 creating array
var buttonColours=["red","blue","green","yellow"];


//7.1.2 You'll need a way to keep track of whether if the game has started or not, so you only call nextSequence() on the first keypress.
var started = false;
...
//7.1.1 detect when a keyboard key has been pressed, when that happens for the first time, call nextSequence()
...
//4.1  jQuery to detect when any of the buttons are clicked and trigger a handler function.
$(".btn").click(function(){
  //4.2 new variable called userChosenColour to store the id of the button that got clicked.
...
function nextSequence()
{
  ...
  //2.2 generating random no between 0 to 3
  var randomNumber=Math.random();
  randomNumber=randomNumber*3;
  randomNumber=Math.floor(randomNumber)+1;

  //2.4 Creating a new variable called randomChosenColour and use the randomNumber from step 2.2 to select a random colour from the buttonColours array.
  var randomChosenColour=buttonColours[randomNumber];
  //2.6 Adding the new randomChosenColour generated in step 2.4 to the end of the gamePattern.
  gamePattern.push(randomChosenColour);

  //3.1 Use jQuery to select the button with the same id as the randomChosenColour
  //3.2  use jQuery to animate a flash to the button selected in step 3.1s
  $("#"+ randomChosenColour).fadeIn(100).fadeOut(100).fadeIn(100);

  //3.3 use Javascript to play the sound for the button colour selected in step 3.1.
...
//6.1Create a new function called animatePress(), it should take a single input parameter called currentColour.
function animatePress(currentColour)
{
  ...
  //6.3 add this "pressed" class to the button that gets clicked inside animatePress().
